...
- Added new DNS seed from karmanode.io
...
- Improve support for start-many with multi karmanode config
- New karmanode rpc commands: stop-many, start-alias, stop-alias, list-conf
- Fixed possible karmanode payments exploit
- Better support for non-specific karmanode payments
- Added karmanode support for regtest
- Randomly sort karmanodes before picking next winner
- Show number of karmanodes in debug window
...
- Add '-disablewallet' mode to let ohmcoind run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with ohmcoind
