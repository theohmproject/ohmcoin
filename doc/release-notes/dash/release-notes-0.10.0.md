...
- saction for Darksend. These are special transactions that require a signature that only the karmanodes can create.
...
- Clients remember which karmanodes they've connected to in the past and won't use them against.
...
- Split up darksend.cpp into karmanode.cpp, activekarmanode.cpp and instantx.cpp
- Added modular ProcessMessages for Darksend, Karmanodes and InstantX
- Client can now join sessions with any other users

0.10.15.20/21 Release notes
---------------------------

- added bloom filters for dsee/dseep broadcasts, moved expensive dsee search, karmanode vote caching
...
- Fixed karmanode payment check for out-of-order blocks
...
- karmanode only take newest dsee now


0.10.15.15 Release notes
------------------------

- fixed fragmentation issue, allow karmanodes to update pubkey2
...
- Fixed the stuck blockchain (due to karmanode payments)
- Improves collateral creation for those getting "invalid collateral"
- Minimum confirmation requirements before karmanode payment.
  Must have more confirmations then there are karmanodes.
...
- Changes the karmanode payments keys, you'll only recieve karmanode
...
- I've made some huge progress with enforcement. Each karmanode will
...
- Fixed some issues with karmanode payments syncing
...
- Fixed some issues with the way the karmanode payment list
  was being handled
- Karmanode confirmation error now displays 200 required instead of 6
...
- security update: This update simply verifies that the signature was
  not forged and that the karmanodes entering the list are authentic.
...
- karmanode constants for communication are much easier to read now
...
- Improved handling of enforcement for bad karmanode lists
- Removed old karmanode override code
...
- Darksend UI is disabled for karmanodes now and titlebar says "[karmanode]"
- Improvement for dealing with splitting large inputs
- Protocol version bump to kick old karmanodes off
...
- Added a spork for enforcing karmanode payments (this will ensure misconfigured
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  I've modified the queuing system, so users wait in a karmanode queue without
...
- removed matching karmanode debugging messages, that's not really an error
- Client now prioritises sending denominated funds back into Darksend. This will
  ...
  configuration.
- fixed a bug where karmanodes send failed transactions
...
- Darksend now checks diskspace before sending anything to a karmanode.
...
- Disabled collateral charging for now. We'll work on this after RC4 is
  released and update the karmanode network after it's working properly. It's
...
- I've switched up the way the karmanode network works.
    1.) Users now will join a random karmanode (1 of the entire list, just
        completely randomly)
    2.) Upon joining if it's the first user, the karmanode will propagate
...
- Fixed some bugs with connecting to the correct karmanodes
...
- Added session IDs for karmanode communication. Clients were getting
  confused when they got messages about other sessions (sometimes happened when
  they all jumped on the same karmanode at once)
- Added a pre-session state where the client will query a random karmanode
...
- Added code to randomly use the top 20 karmanodes, this can dynamically be
...
- After successful transactions clients will now automatically attempt another
  session on a random karmanode, then repeat until they get any kind of error
  or run out of funds that need to be processed.
- Fixed a change address reuse issue
- Fixed an issue with the compatible join algorithm (Karmanodes will only join
...
- Fixed an issue where clients weren't connected to the correct karmanode
- Fixed karmanode relay issues
...
- This version automatically resets the karmanode state machine after a short
  period of inactivity.
- Updated protocol version to kick old karmanodes off
...
- Found a communication mix up where clients would see messages from the wrong
  karmanode and think it was theirs, also causing collateral fees
...
- incremented protocol version to force karmanode updates
...
- Client will only submit 1 transaction into the pool fixing possible
  anonymity issues
- Karmanodes will only merge compatible transactions using the same
...
- Added possible fix for karmanode list syncing
...
- Karmanodes now reset themselves when they give "entries is full".
...
on the same karmanode for testing purposes:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -darksendrounds=8 -usekarmanode="192.168.56.102:51474"
/darkcoin-qt -datadir=/home/user/.darkcoin2 -listen=0 -darksendrounds=8 -usekarmanode="192.168.56.102:51474"

and even disable darksend auto-denom if wanted:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -darksendrounds=8 -usekarmanode="192.168.56.102:51474" -disabledarksend=1
...
- Karmanodes should change every block now (missed a mod=10 last time)
...
- Karmanodes should change every block now
...
- usekarmanode option, will override active karmanode (only in RC, just for testing)
...
- Fixed payout issues (karmanode consessus was paying out to vout(0) by default)
- Improved DarksendInput add entry verification. Karmanodes will now reject
...
- Fixed a signing bug with the karmanode voting system causing a bunch of issues
- Updated unit tests
