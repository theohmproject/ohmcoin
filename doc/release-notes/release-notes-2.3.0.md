...
#### karmanode command
The `karmanode` RPC command has been re-worked to ease it's usage and return valid JSON in it's results. The following is an overview of the changed command parameters:
...
[
  {
    ...
    "addr": addr,      (string) Karmanode PIVX address
