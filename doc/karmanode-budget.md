Karmanode Budget API
=======================

Ohm now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
* finalized budget voting - karmanodes that agree with the finalization will vote on that budget
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<ohmc_address\> \<monthly_payment_ohmc\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ OHMC, _12_ times over the course of a year totaling _24000_ OHMC.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<ohmc_address\> \<monthly_payment_ohmc\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` OHMC.
 ...
 * vote-many          - Vote on a Ohm initiative
 * vote-alias         - Vote on a Ohm initiative
 * vote               - Vote on a Ohm initiative/budget
