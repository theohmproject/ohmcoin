...
class Node :
	def __init__(self, app, nodeIndex) :
		...
		self._ohmcd = self._app.getOhmcd()
		self._ohmcCli = self._app.getOhmcCli()
		self._daemonProcess = None

	def createDataDir(self, nodeCount, karmaNodePrivateKey = None) :
		makeDirs(self._nodePath)
		writeFile(
			os.path.join(self._nodePath, 'ohmc.conf'),
			self._generateOhmcConf(nodeCount, karmaNodePrivateKey)
		)

	def startNode(self) :
		self._daemonProcess = subprocess.Popen([ self._ohmcd, '-datadir=' + self._nodePath, '-daemon' ])
	...
	def executeCli(self, *args) :
		...
		output = executeCommand(self._ohmcCli, '-datadir=' + self._nodePath, *normalizedArgs)
	...
	def _generateOhmcConf(self, nodeCount, masterNodePrivateKey) :
		result = ""
		result += "regtest=1\n"
		result += "server=1\n"
		result += "debug=1\n"
...
class Application :
	...
	def _setup(self) :
		...
		self._ohmcd = os.getenv('OHMCD', None)
		if not self._ohmcd :
			die('Undefined OHMCD')
		self._ohmcCli = os.getenv('OHMCCLI', None)
		if not self._ohmcCli :
			die('Undefined OHMCCLI')
		print('ohmcd: %s' % (self._ohmcd))
	...
	def _createNodes(self) :
		...
		if nodesExist :
			...
			for i in range(1, self._nodeCount) :
				node = Node(self, i)
				self._nodeList.append(node)
				key = controllingNode['karmaNodePrivateKeyList'][i - 1]
		...
		for i in range(1, self._nodeCount) :
			result = self._nodeList[0].executeCli('startkarmanode', 'alias', 'false', getNodeAlias(i))
			print(result['output'])
	...
	def _createControllingNode(self) :
		...
		karmaNodePrivateKeyList = []
		karmaNodeConfig = ''
		for i in range(1, self._nodeCount) :
			key = node.executeCli('karmanode', 'genkey')['json']
			karmaNodePrivateKeyList.append(key)
			...
			checkError(result)
			tx = result['json']
			outputs = node.executeCli('karmanode', 'outputs')
		...
		return {
			'node' : node,
			'karmaNodePrivateKeyList' : karmaNodePrivateKeyList,
		}
	...
	def _syncKarmaNodes(self, timeoutSeconds = 60) :
		...
		while timeoutSeconds < 0 or time.time() - startTime < timeoutSeconds :
			...
			for i in range(1, len(self._nodeList)):
				if not self._nodeList[i].isMasterNodeSynced() :
					#print('KN %d status %s' % (i, self._nodeList[i].executeCli('knsync', 'status')['output']))
					allSynced = False
					break
			if allSynced :
				return True
			time.sleep(1)
		print('_syncKarmaNodes failed')
		return False
