...
CzOHMCTracker::CzOHMCTracker(std::string strWalletFile)
{
...
CzOHMCTracker::~CzOHMCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzOHMCTracker::Init()
{
...
bool CzOHMCTracker::Archive(CMintMeta& meta)
{
...
bool CzOHMCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzOHMCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzOHMCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzOHMCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzOHMCTracker::GetSerialHashes()
{
...
CAmount CzOHMCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzOHMCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzOHMCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzOHMCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzOHMCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzOHMCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzOHMCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzOHMCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzOHMCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzOHMCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzOHMCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOHMCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOHMCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzOHMCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzOHMCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzOHMCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzOHMCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzohmc from DB\n", __func__, listDeterministicDB.size());
    }
