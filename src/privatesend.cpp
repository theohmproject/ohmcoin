...
void CPrivateSendPool::ProcessMessagePrivateSend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //PrivateSend Accept Into Pool
        ...
        if (dsq.ready) {
            ...
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("privatesend", "dsq -- Karmanode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
...
//
// Check the PrivateSend progress and send client updates if a Karmanode
//
...
void CPrivateSendPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        if (!obfuScationSigner.SetKey(strMasterNodePrivKey, strError, key2, pubkey2)) {
            LogPrintf("CPrivateSendPool::Check() - ERROR: Invalid Karmanodeprivkey: '%s'\n", strError);
            return;
        }
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat OHMC and make it unusable. To
...
//
// Execute a PrivateSend denomination via a Karmanode.
// This is only ran from clients
//
void CPrivateSendPool::SendPrivateSendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount)
{
    if (fMasterNode) {
        LogPrintf("CPrivateSendPool::SendPrivateSendDenominate() - PrivateSend from a Karmanode is not supported currently.\n");
        return;
    }
    ...
    if (!sessionFoundKarmanode) {
        LogPrintf("CPrivateSendPool::SendPrivateSendDenominate() - No Karmanode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
...
// Incoming message from Karmanode updating the progress of PrivateSend
...
bool CPrivateSendPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            ...
            LogPrintf("CPrivateSendPool::StatusUpdate - entry not accepted by Karmanode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
...
bool CPrivateSendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CPrivateSendPool::DoAutomaticDenominating - No Karmanodes detected\n");
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeOhmAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
bool CPrivateSendQueue::Sign()
{
    ...
    if (!obfuScationSigner.SetKey(strMasterNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CPrivateSendQueue():Relay - ERROR: Invalid Karmanodeprivkey: '%s'\n", errorMessage);
        return false;
    }
...
bool CPrivateSendQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        if (!obfuScationSigner.VerifyMessage(pmn->pubKeyKarmanode, vchSig, strMessage, errorMessage)) {
            return error("CPrivateSendQueue::CheckSignature() - Got bad Karmanode address signature %s \n", vin.ToString().c_str());
        }
