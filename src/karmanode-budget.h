...
//
// CFinalizedBudgetVote - Allow a karmanode node to vote and broadcast throughout the network
//
...
public:
    ...
    std::map<uint256, CBudgetProposalBroadcast> mapSeenKarmanodeBudgetProposals;
    std::map<uint256, CBudgetVote> mapSeenKarmanodeBudgetVotes;
    std::map<uint256, CBudgetVote> mapOrphanKarmanodeBudgetVotes;
    ...
    void ClearSeen()
    {
        mapSeenKarmanodeBudgetProposals.clear();
        mapSeenKarmanodeBudgetVotes.clear();
    ...
    void Clear()
    {
        ...
        mapSeenFinalizedBudgets.clear();
        mapSeenFinalizedBudgetVotes.clear();
        mapOrphanKarmanodeBudgetVotes.clear();
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion)
    {
        READWRITE(mapSeenKarmanodeBudgetProposals);
        READWRITE(mapSeenKarmanodeBudgetVotes);
        READWRITE(mapSeenFinalizedBudgets);
        READWRITE(mapSeenFinalizedBudgetVotes);
        READWRITE(mapOrphanKarmanodeBudgetVotes);
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (karmanode only)
...
//
// Budget Proposal : Contains the karmanode votes for each budget
//
