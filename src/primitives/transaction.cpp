...
bool COutPoint::IsKarmanodeReward(const CTransaction* tx) const
{
    if(!tx->IsCoinStake())
        return false;
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // Ohmcoin: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
