...
#include "ui_zohmccontroldialog.h"
...
std::set<std::string> ZOhmcControlDialog::setSelectedMints;
std::set<CMintMeta> ZOhmcControlDialog::setMints;

ZOhmcControlDialog::ZOhmcControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOhmcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZOhmcControlDialog::~ZOhmcControlDialog()
{
    delete ui;
}

void ZOhmcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOhmcControlDialog::updateList()
{
...
void ZOhmcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOhmcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZOhmc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOhmcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZOhmcControlDialog::GetSelectedMints()
{
...
void ZOhmcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
