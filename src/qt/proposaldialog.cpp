...
ProposalDialog::ProposalDialog(Mode mode, QWidget* parent) : QDialog(parent), ui(new Ui::ProposalDialog), mapper(0), mode(mode), counter(0)
{
    ...
    ui->addressEdit->setPlaceholderText(tr("The Ohmcoin public address that will receive the funds"));
    ui->addressEdit->setToolTip(tr("The Ohmcoin public address that will receive the funds"));
...
bool ProposalDialog::validateProposal()
{
    ...
    if (!karmanodeSync.IsBlockchainSynced()) strError = "Must wait for client to sync with karmanodenode network. Try again in a minute or so.";
    ...
    std::string address = ui->addressEdit->text().toStdString();
    if (!IsValidDestinationString(address)) strError = "Invalid Ohmcoin address";
