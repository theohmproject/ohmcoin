...
void MultiSendDialog::updateCheckBoxes()
{
    ...
    ui->multiSendKarmanodeCheckBox->setChecked(pwalletMain->fMultiSendKarmanodeReward);
}
...
void MultiSendDialog::on_viewButton_clicked()
{
    ...
    if (pwalletMain->isMultiSendEnabled()) {
        if (pwalletMain->fMultiSendStake && pwalletMain->fMultiSendKarmanodeReward)
            strMultiSendPrint += "MultiSend Active for Stakes and Karmanode Rewards\n";
        else if (pwalletMain->fMultiSendStake)
            strMultiSendPrint += "MultiSend Active for Stakes\n";
        else if (pwalletMain->fMultiSendKarmanodeReward)
            strMultiSendPrint += "MultiSend Active for Karmanode Rewards\n";
    } else
...
void MultiSendDialog::on_activateButton_clicked()
{
    ...
    if (pwalletMain->vMultiSend.size() < 1)
    ...
    else if (!(ui->multiSendStakeCheckBox->isChecked() || ui->multiSendKarmanodeCheckBox->isChecked())) {
        strRet = "Need to select to send on stake and/or karmanode rewards\n";
        ...
        pwalletMain->fMultiSendKarmanodeReward = ui->multiSendKarmanodeCheckBox->isChecked();

        CWalletDB walletdb(pwalletMain->strWalletFile);
        if (!walletdb.WriteMSettings(pwalletMain->fMultiSendStake, pwalletMain->fMultiSendKarmanodeReward, pwalletMain->nLastMultiSendHeight))
