...
CObfuScationRelay::CObfuScationRelay(CTxIn& vinKarmanodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinKarmanode = vinKarmanodeIn;
    vchSig = vchSigIn;
...
std::string CObfuScationRelay::ToString()
{
    ...
    info << "vin: " << vinKarmanode.ToString() << " nBlockHeight: " << (int)nBlockHeight << " nRelayType: " << (int)nRelayType << " in " << in.ToString() << " out " << out.ToString();
...
{
    CKarmanode* pmn = mnodeman.GetKarmanodeByRank(nRank, nBlockHeight, ActiveProtocol());
